...
function getSingleInitial(fullName: string): string {
...
export function LoginPage(): React.JSX.Element {
    ...
    return (
      <div className="flex h-screen w-screen items-center justify-center bg-[#F2F2F7] dark:bg-slate-950 p-6 select-none relative overflow-hidden">
        ...
        <div className="relative w-full max-w-2xl flex flex-col items-center page-enter">
          ...
          <p className="text-sm font-extrabold text-text-secondary dark:text-slate-400 mt-4 mb-8">
          ...
          {isLoadingUsers ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 w-full">
              {users.map((user) => {
                ...
                const initial = getSingleInitial(user.full_name)
                ...
                return (
                  ...
                  >
                    ...
                    {/* Avatar circle with single bold letter & role badge */}
                    <div className="relative">
                      ...
                      <div
                        ...
                        className="relative w-16 h-16 rounded-full text-white font-black text-2xl flex items-center justify-center border-2 border-white/40 shadow-lg group-hover:scale-105 transition-transform"
                      >
                        {initial}
                        <div className="absolute -bottom-1 -right-1 w-6 h-6 rounded-full bg-slate-900 border-2 border-white flex items-center justify-center text-xs shadow-md">
                          {meta.icon}
                        </div>
                      </div>
                    </div>
          ...
          <button
            ...
            className="mt-8 text-xs font-extrabold text-slate-500 dark:text-slate-400 hover:text-accent transition-colors flex items-center gap-1.5 px-4 py-2 rounded-full bg-white/60 dark:bg-slate-900/60 border border-gray-200 dark:border-slate-800 shadow-sm"
          >
  ...
  const initial = getSingleInitial(selectedUser.full_name)
      ...
      <div className="relative w-full max-w-md bg-white/95 dark:bg-slate-900/95 backdrop-blur-2xl p-8 rounded-3xl shadow-2xl border border-gray-200/80 dark:border-slate-800 flex flex-col items-center page-enter">
        ...
        <div className="relative mb-3">
          <div
            ...
            className="absolute -inset-1 rounded-full blur-md opacity-60 animate-pulse pointer-events-none"
          />
          <div
            style={{
              background: 'linear-gradient(135deg, var(--color-accent, #0A84FF) 0%, var(--color-accent-hover, #00C6FF) 100%)'
        ...
        <h2 className="text-xl font-black text-slate-900 dark:text-slate-100">{selectedUser.full_name}</h2>
        <span
          className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-extrabold border mt-1.5 ${meta.color}`}
        >
          {meta.icon}
          <span>{t(meta.label)}</span>
        </span>

        <p className="text-xs font-extrabold text-slate-500 dark:text-slate-400 mt-4 mb-6">{t('أدخل رمز PIN الخاص بك')}</p>

        {/* Masked PIN Dots with Active Glow & Scale Effect */}
          ...
          {Array.from({ length: 4 }).map((_, index) => {
            ...
            return (
              <div
                key={`pin-dot-${index}`}
                style={{
                  backgroundColor: hasValue ? 'var(--color-accent, #0A84FF)' : undefined,
                  borderColor: hasValue ? 'var(--color-accent, #0A84FF)' : undefined,
                  boxShadow: hasValue ? '0 0 12px var(--color-accent, #0A84FF)' : undefined
                }}
